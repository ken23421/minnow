            ...
            if 'degraded' in output:
                ...
                for line2 in output.splitlines():
                    if 'active sync' in line2:
                        first_active_disk[md] = line2.split()[6][5:8]
                        break
    mdstat.close()
    if not degraded:
        log.flush()
...
if __name__ == "__main__":
    ...
    for d in ['sda', 'sdb']:
        if d not in content:
            print >> log, 'python /opt/hpe/minnow/bin/md_resync_trigger.py ' + d
            os.system('python /opt/hpe/minnow/bin/md_resync_trigger.py ' + d)
    log.flush()
